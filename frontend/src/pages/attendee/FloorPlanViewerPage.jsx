...
import styles from './FloorPlanViewerPage.module.css'
...
export default function FloorPlanViewerPage() {
  ...
  const activeExpos = registrations.filter(r => r.expoId?.status === 'Published' || r.expoId?.status === 'Live')
  const currentExpoId = selectedExpoId || activeExpos[0]?.expoId?._id

  return (
    <div className={styles.page}>
      <h1 className={styles.title}>Floor Plan Viewer</h1>
      <p className={styles.sub}>Explore booth layouts for your registered expos</p>

      {isLoading ? <Skeleton height={48} /> :
       !activeExpos.length ? (
        <div className={styles.empty}>
          <p>You're not registered for any active expos. <Link to="/attendee/events">Browse events →</Link></p>
        </div>
      ) : (
        <>
          {activeExpos.length > 1 && (
            <div className={styles.expoSelector}>
              {activeExpos.map(reg => (
                <button key={reg.expoId?._id}
                  className={`${styles.expoTab} ${currentExpoId === reg.expoId?._id ? styles.activeExpo : ''}`}
                  onClick={() => setSelectedExpoId(reg.expoId?._id)}>
                  {reg.expoId?.name}
                </button>
              ))}
            </div>
